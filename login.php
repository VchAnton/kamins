<?php
session_start();
$pass='pavelgrinchuk'; // Пароль для входа в Админку
...
if((isset($_POST['slovo'])||isset($_POST['sekret']))||($_SESSION['sekret']==md5($pass))){
    ...
    line-height: 24px;
    font-size: 31px;
    font-family: sans-serif;">Авторизация</div>
...
if($_GET['mode']=='7'){
	...
	$imgreg = "/[\"|\-–(']((.*\\/\\/|)([\\/a-z0-9_%]+\\.(jpg|png|gif|webp|svg|jpeg|ico|tiff|eps)))[\"|\-)']/"; 
	...
	for ($i=0; $i< count($mycss); $i++) {
		...
		$imgreg = '/(?:url\(["\']?)([^"\')]+?\.(?:jpg|jpeg|png|gif|webp|svg))(?:["\']?\))?/';
...
if($_GET['mode']=='3'){
	...
	echo('<div class="kartinka"><a href="login.php?mode=1&img='.$imgname.'"><img src="'.$imgname.'?'.rand(1, 32000).'" alt="Image"></a><br>'.$imgname.'<br>');
echo '<p>Path: ' . htmlspecialchars($imgname) . '</p>';
...
if(!isset($_GET['mode'])){
	echo('<div id="help"><p><br><h2>Вы находитесь в панели управления сайтом.</h2><p>Да это и есть админка. <p><p>
		Слева вверху располагается выпадающий список, в котором выбирается та страница, в котрую необходимо вносить правки, index.html — это главная страница вашего сайта, все остальные страницы — внутренние, их названия видно в адрестной строке браузера после слэша "/".
		<p>Правее вверху находятся кнопки для выбора того контента, которых необходимо редактировать. При нажатии на "Текст" вы сможете редактировать тексты на сайте, просто нажав по нужному абзацу или слову и заменив его на новое, после чего сохранить изменения.<p>
		При нажатии на кнопку "Картинки" можно менять абсолютно любую графику на новую, просто нажав по той, которую нужно заменить, загрузить на ее место новую и сохранить изменения. 
		<hr>
		<p> ОБРАТИТЕ ВНИМАНИЕ! Размер загружаемой картинки должен быть такой же как и у старой. Тоесть если старая была 600х400 пикселей, то и новая тоже должна быть 600х400 пикселей. Это важно для корректности отображения на сайте и быстроты его загрузки.<hr>
		<p> По кнопкам HTML и CSS JS лучше не нажимать, поскольку изменения в функционал, что там находится, могут осуществлять люди, которые понимают код. Чтобы не поломать сайт, внеся туда некорректные изменения или случайно удалив всего один символ, что отобразится на работе всего сайта, без помощи впециалиста туда не переходите. 
		<p> На вкладке HTML можно вставлять код метрик/аналитик/фейсбук-пикселей/онлайн консультантов и прочих штук, требующих вставки кода. Подробно об этом показано в видео.
<br><br><br><p><a href="https://youtu.be/Xkc5KtJ9d7g" target="_blank">Видео инструкция по админке тут —  просто нажмите по этому тексту.</a><br><br>
		<p>У вас остались вопросы? Необходима помощь с внесением изменений? <p>Тогда <a href="http://grincnuk@gmail.com" target="_blank">вот мой сайт — pavelgrinchuk.com просто нажмите по этому тексту.</a> <p>Так же, можете написать мне на почту grincnuk@gmail.com</div>');
};
